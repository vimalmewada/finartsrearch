<!-- Page Title -->
<div class="page-title dark-background">
    <div class="container position-relative">
        <h1>Complaint Data </h1>
        <nav class="breadcrumbs">
            <ol>
                <li><a href="/">Home</a></li>
                <li class="current">
                    Complaint Data</li>
            </ol>
        </nav>
    </div>
</div><!-- End Page Title -->

<div class="container my-4">
        <div class="table-responsive">
            <table class="table table-bordered table-hover align-middle">
                <thead class="table-dark text-center">
                    <tr>
                        <th colspan="8">Data for the Month Ending August 2025</th>
                    </tr>
                    <tr>
                        <th>Sr. No.</th>
                        <th>Received from</th>
                        <th>Pending at the end of last month</th>
                        <th>Received</th>
                        <th>Resolved</th>
                        <th>Total Pending</th>
                        <th>Pending complaints &gt; 3 months</th>
                        <th>Average Resolution time (in days)</th>
                    </tr>
                </thead>
                <tbody>
                    <tr>
                        <td>1</td>
                        <td>Directly from Investors</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>-</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>SEBI (SCORES)</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>-</td>
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>Other Sources</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>-</td>
                    </tr>
                    <tr>
                        <td></td>
                        <td><strong>Grand Total</strong></td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>0</td>
                        <td>-</td>
                    </tr>

                    <!-- Monthly Trend Heading -->
                    <tr class="table-secondary text-center">
                        <td colspan="8"><strong>Trend of Monthly Complaints</strong></td>
                    </tr>
                    <tr>
                        <th>Sr. No.</th>
                        <th>Month</th>
                        <th>Carried forward</th>
                        <th colspan="2">Received</th>
                        <th colspan="2">Resolved</th>
                        <th>Pending</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>September 2024</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>October 2024</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>November 2024</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>4</td>
                        <td>December 2024</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>5</td>
                        <td>January 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>6</td>
                        <td>February 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>7</td>
                        <td>March 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>8</td>
                        <td>April 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>9</td>
                        <td>May 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>10</td>
                        <td>June 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>11</td>
                        <td>July 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>12</td>
                        <td>August 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                      <tr>
                        <td>12</td>
                        <td>September 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                      <tr>
                        <td>12</td>
                        <td>October 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                      <tr>
                        <td>12</td>
                        <td>November 2025</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>

                    <!-- Annual Trend Heading -->
                    <tr class="table-secondary text-center">
                        <td colspan="8"><strong>Trend of Annual Complaints</strong></td>
                    </tr>
                    <tr>
                        <th>Sr. No.</th>
                        <th>Year</th>
                        <th>Carried forward</th>
                        <th colspan="2">Received</th>
                        <th colspan="2">Resolved</th>
                        <th>Pending</th>
                    </tr>
                    <tr>
                        <td>1</td>
                        <td>2023-24</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>2</td>
                        <td>2024-25</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                    <tr>
                        <td>3</td>
                        <td>2025-26</td>
                        <td>0</td>
                        <td colspan="2">0</td>
                        <td colspan="2">0</td>
                        <td>0</td>
                    </tr>
                </tbody>
            </table>
        </div>
    </div>





</div>
</div>